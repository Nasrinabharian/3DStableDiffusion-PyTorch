...
class DownBlock(nn.Module):
    ...
    def forward(self, x, t_emb=None, context=None):
        ...
        for i in range(self.num_layers):
            ...
            if self.t_emb_dim is not None:
                out = out + self.t_emb_layers[i](t_emb)[:, :, None, None, None]#changed
            out = self.resnet_conv_second[i](out)
            out = out + self.residual_input_conv[i](resnet_input)
            
            if self.attn:
                # Attention block of Unet
                batch_size, channels, d, h, w = out.shape #changed
                in_attn = out.reshape(batch_size, channels, d * h * w) #changed
                in_attn = self.attention_norms[i](in_attn)
                in_attn = in_attn.transpose(1, 2)
                out_attn, _ = self.attentions[i](in_attn, in_attn, in_attn)
                out_attn = out_attn.transpose(1, 2).reshape(batch_size, channels, d, h, w)#changed
                out = out + out_attn
...
class MidBlock(nn.Module):
    ...
    def forward(self, x, t_emb=None, context=None):
        ...
        if self.t_emb_dim is not None:
            out = out + self.t_emb_layers[0](t_emb)[:, :, None, None, None]#changed
        ...
        for i in range(self.num_layers):
            ...
            if self.t_emb_dim is not None:
                out = out + self.t_emb_layers[1](t_emb)[:, :, None, None, None]
                #out = out + self.t_emb_layers[i + 1](t_emb)[:, :, None, None, None] #changed
...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, t_emb_dim,
                 up_sample, num_heads, num_layers, attn, norm_channels, cross_attn=False, context_dim=None):
        super().__init__()
        self.num_layers = num_layers
        ...
        self.up_sample_conv = nn.ConvTranspose3d(in_channels, in_channels,
                                                 4, 2, 1) \
            if self.up_sample else nn.Identity() #changed
