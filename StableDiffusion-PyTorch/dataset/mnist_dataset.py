...
class MnistDataset(Dataset):
    ...
    def load_images(self, im_path):
        ...
        for d_name in tqdm(os.listdir(im_path)):
            fnames = glob.glob(os.path.join(im_path, d_name, '*.{}'.format('nii')))
            print(f"Found files: {fnames} in directory: {d_name}")
            if len(fnames) == 0:
                print(f"Warning: No NIfTI files found in {os.path.join(im_path, d_name)}")
            ...
            for fname in fnames:
                ...
                nifti_img1 = torch.from_numpy(nifti_img1)
                nifti_img1 = torch.unsqueeze(nifti_img1, 0)
                nifti_img1 = nifti_img1.numpy()
                #print("nifti ", nifti_img1.shape)
                ims.append(nifti_img1)
    ...
    def __getitem__(self, index):
        ...
        if self.use_latents:
            ...
            # Check if the image is a NIfTI (numpy array) or a regular image file (jpg, png)
            if isinstance(self.images[index], np.ndarray):
                # If NIfTI, the image is already loaded as a NumPy array
                im_tensor = torch.tensor(self.images[index], dtype=torch.float32)
            else:
